...
from comer.utils.utils import (WRRRecorder, Hypothesis, ce_loss,
                               to_bi_tgt_out)
...
class LitCoMER(pl.LightningModule):
    ...
    def validation_step(self, batch: Batch, _):
        ...
        self.wrr_recorder([h.seq for h in hyps], batch.indices)
        self.log(
            "val_WRR",
            self.wrr_recorder,
            prog_bar=True,
    ...
    def test_epoch_end(self, test_outputs) -> None:
        wrr = self.wrr_recorder.compute()
        print(f"Validation WRR: {wrr}")
