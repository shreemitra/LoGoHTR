...
def collate_fn(batch):
    ...
    # Initialize the tensor for 3-channel images
    x = torch.zeros(n_samples, 3, max_height_x, max_width_x)  # 3 channels
    x_mask = torch.ones(n_samples, max_height_x, max_width_x, dtype=torch.bool)
    
    for idx, s_x in enumerate(images_x):
        # Assign image data while keeping the 3 channels intact
