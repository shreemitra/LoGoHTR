...
class DenseNet(nn.Module):
    ...
    def forward(self, x, x_mask):
        out = self.conv1(x)
        out_first_layer = self.norm1(out)
...
class Encoder(pl.LightningModule):
    def __init__(self, d_model: int, growth_rate: int, num_layers: int):
        ...
        print("🔄 Loading pretrained weights from BarlowTwins backbone...")
        self.model.load_state_dict(
            torch.load("/home2/shree.mitra23m/CoMER/comer/model/BarlowTwins_HTR_backbone_step290000.pth"),
            strict=True
        )
        ...
        for name, param in self.model.named_parameters():
            ...
            print(f"Un-Frozen: {name}")
        print("✅ All parameters are now Un-frozen (trainable).")
    ...
    def forward(
        ...
        feature, _, mask = self.model(img, img_mask)
